...
# @TODO PENTRU CONFIG FILE 1 NU VA FI NICIO DEIFERENTA DEOARECE SINGURA CARE AR PUTEA FI DEAD STATE ESTE CHIAR STAREA FINALA Q3--Q5
...
adv_dfa_minimized_states = []
adv_dfa_minimized_transitions = []
eliminated = []
lungalph = len(sigma)
for i in range(len(dfa_minimized_states)):
    nr = 0
    for chr in sigma:
        if [dfa_minimized_states[i], chr, dfa_minimized_states[i]] in dfa_minimized_transitions:
            nr += 1
    if nr == lungalph and dfa_minimized_states[i] not in dfa_minimized_finals:
        eliminated.append(dfa_minimized_states[i])
        continue
    else:
        adv_dfa_minimized_states.append(dfa_minimized_states[i])

dfa_minimized_states = adv_dfa_minimized_states
for j in range(len(dfa_minimized_transitions)):
    if dfa_minimized_transitions[j][0] in eliminated or dfa_minimized_transitions[j][2] in eliminated:
        continue
    else:
        adv_dfa_minimized_transitions.append(dfa_minimized_transitions[j])

dfa_minimized_transitions = adv_dfa_minimized_transitions
